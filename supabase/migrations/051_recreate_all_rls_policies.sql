-- Recreate all RLS policies that were dropped when user_has_clan_role function was recreated
-- This migration restores RLS policies across all tables
...
-- ===== GROUP_MEMBERS TABLE =====
CREATE POLICY "clan_members_update" ON group_members
  FOR UPDATE USING (
    user_has_clan_role(group_id, auth.uid(), ARRAY['admin'::text, 'officer'::text])
  );

CREATE POLICY "clan_members_delete" ON group_members
  FOR DELETE USING (
    user_has_clan_role(group_id, auth.uid(), ARRAY['admin'::text, 'officer'::text])
  );
...
CREATE POLICY "Clan members can view roster" ON party_roster
  FOR SELECT USING (
    user_has_clan_role((SELECT group_id FROM parties WHERE id = party_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );

CREATE POLICY "Officers can manage roster" ON party_roster
  FOR INSERT WITH CHECK (
    user_has_clan_role((SELECT group_id FROM parties WHERE id = party_id), auth.uid(), ARRAY['officer'::text, 'admin'::text])
  );
...
-- ===== SIEGE_ROSTER TABLE =====
CREATE POLICY "Clan members can view roster" ON siege_roster
  FOR SELECT USING (
    user_has_clan_role((SELECT group_id FROM siege_events WHERE id = event_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );

CREATE POLICY "Members can withdraw or officers manage" ON siege_roster
  FOR UPDATE USING (
    (user_id = auth.uid()) OR user_has_clan_role((SELECT group_id FROM siege_events WHERE id = event_id), auth.uid(), ARRAY['officer'::text, 'admin'::text])
  );
...
CREATE POLICY "Clan members can view inventory" ON bank_inventory
  FOR SELECT USING (
    user_has_clan_role((SELECT group_id FROM guild_banks WHERE id = bank_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );

CREATE POLICY "Officers can manage inventory" ON bank_inventory
  FOR INSERT WITH CHECK (
    user_has_clan_role((SELECT group_id FROM guild_banks WHERE id = bank_id), auth.uid(), ARRAY['officer'::text, 'admin'::text])
  );

-- ===== BANK_TRANSACTIONS TABLE =====
CREATE POLICY "Clan members can view transactions" ON bank_transactions
  FOR SELECT USING (
    user_has_clan_role(group_id, auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );

CREATE POLICY "Members can create transactions" ON bank_transactions
  FOR INSERT WITH CHECK (
    user_has_clan_role(group_id, auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );
...
CREATE POLICY "Clan members can view buildings" ON freehold_buildings
  FOR SELECT USING (
    user_has_clan_role((SELECT group_id FROM freeholds WHERE id = freehold_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );
...
CREATE POLICY "Creators and officers can update caravans" ON caravan_events
  FOR UPDATE USING (
    (creator_id = auth.uid()) OR user_has_clan_role(group_id, auth.uid(), ARRAY['officer'::text, 'admin'::text])
  );
...
CREATE POLICY "Clan members can view escorts" ON caravan_escorts
  FOR SELECT USING (
    user_has_clan_role((SELECT group_id FROM caravan_events WHERE id = event_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );
...
CREATE POLICY "Creators can manage waypoints" ON caravan_waypoints
  FOR INSERT WITH CHECK (
    (SELECT creator_id FROM caravan_events WHERE id = event_id) = auth.uid()
  );
...
CREATE POLICY "View builds based on visibility" ON builds
  FOR SELECT USING (
    (visibility = 'public') OR (creator_id = auth.uid()) OR user_has_clan_role(group_id, auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );

-- ===== BUILD_COMMENTS TABLE =====
CREATE POLICY "Comments visible with build" ON build_comments
  FOR SELECT USING (
    (SELECT visibility FROM builds WHERE id = build_id) = 'public' OR (SELECT creator_id FROM builds WHERE id = build_id) = auth.uid() OR user_has_clan_role((SELECT group_id FROM builds WHERE id = build_id), auth.uid(), ARRAY['admin'::text, 'officer'::text, 'member'::text])
  );
...
INSERT INTO migration_history (filename) VALUES ('051_recreate_all_rls_policies.sql');
